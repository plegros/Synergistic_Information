...
Hence, firm $2$ offers a mechanism $(p,\a)$ to solve $\max_{\{p(\cdot),\a(\cdot)\}}\int_{\t_0}^{\overline t}\left[(1-\a(\t))(1+\t)X-p(\t)\right]f(\t)dt$ subject to the IR and IC constraints. By using $-p(\t)=\a(\t)(1+\t)X-U(\t)$, the problem can be rewritten as
\begin{align*}
...
%
\begin{align*}
\max_{\{z(\t)\}} &\int_{\t\in \N}\left( (1+\t)X-\a(\t)X\frac{1-F(\t)}{f(\t)}\right)f(\t)d\t.
\end{align*}
%
Which is maximized when $\a(\t)$ is minimized.
\todo[inline]{Patrick là je ne suis pas sur: This is achieved for $\a=\a(\t_0)=\frac{X-L(\t_0)}{(1+\t_0)X}$}
%
The expected payoff to firm $2$ is (using $T=L(\t_0)$ and $\a=\frac{X-L(\t_0)}{(1+\t_0)X}$)
%
\begin{equation}\label{eq:V-t0}
\begin{aligned}
  V(\t_0):=\begin{cases}
    \int_0^{\frac{c}{X}} -L(\t_0)f(\t)d\t +\int_{\frac{c}{X}}^{\frac{c}{s}}(\t X-c-L(\t_0))f(\t)d\t+\int_{\frac{c}{s}}^{\t_0}(\t X-c-L(\t_0)+L(\t))f(\t)d\t\\
    +\int_{\t_0}^{\overline \t}\left[(1+\t)X-c-\frac{X-L(\t_0)}{1+\t_0}\frac{1-F(\t)}{f(\t)}\right]f(\t)d\t ~~\textrm{if}~~ \t_0\geq\frac{c}{s}\\
    \\
    \int_0^{\frac{c}{X}} -L(\t_0)f(\t)d\t +\int_{\frac{c}{X}}^{\t_0}(\t X-c-L(\t_0))f(\t)d\t+\int_{\t_0}^{\overline \t}\left[(1+\t)X-c-\frac{X-L(\t_0)}{1+\t_0}\frac{1-F(\t)}{f(\t)}\right]f(\t)d\t~~\textrm{if}~~ \frac{c}{s}\geq\t_0\geq\frac{c}{X}\\
    \\
    \int_{\t_0}^{\overline \t}\left[(1+\t)X-c-\frac{X-L(\t_0)}{1+\t_0}\frac{1-F(\t)}{f(\t)}\right]f(\t)d\t~~\textrm{if}~~ \frac{c}{X}\geq\t_0\\
    \end{cases}
\end{aligned}
  \end{equation}
